<x-layout>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="kode_jurusan" id="kode_jurusan" class="form-select" required>
                                    <option value="">Pilih Kode Konsentrasi Keahlian</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="no_jrs" class="form-label">No Konsentrasi Keahlian</label>
    ...
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <h2>Kelas yang Anda ajar untuk Mata Pelajaran {{ $mapel_id }} :</h2>
        <a href="{{ url()->previous() }}" class="btn btn-secondary">Kembali</a>
    </div> 
    @if($kelasData->isEmpty())
        <div class="alert alert-warning">Belum ada kelas yang Anda ajar untuk mata pelajaran ini.</div>
    @else
        <div class="row">
            @foreach ($kelasData as $data)
                <div class="col-md-4">
                    <div class="card shadow-sm border-0">
                        <div class="card-body" style="background-color: #435ebe; color: white; border-radius: 10px;">
                            <h5 class="card-title">{{ $data->kelas }} {{ $data->jurusan->kode_jurusan }} {{ $data->no_jrs }}</h5>
                            <p>Jumlah Siswa : {{ $data->siswa->count() }}</p>
                            <a href="{{ route('kelas.siswa', ['kelas' => $data->id]) }}" class="btn btn-light rounded-pill">Lihat Data</a>
                        </div>
                    </div>
                </div>
